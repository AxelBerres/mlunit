...
assert_true(strcmp('test_result run=1 errors=1 failures=0', summary(self.result)));
self.result = add_failure(self.result, mock_test('test_method'), 'foo failure');
assert_true(strcmp('test_result run=1 errors=1 failures=1', summary(self.result)));
